...
export const genUserOpHash = async (chain: Chain, op: UserOperationStruct) => {
...
export const buildZkAdminData = (admin: GoogleZkAdmin, accountHash: string) => {
  const adminData = admin.interface.encodeFunctionData("linkAccount", [
    accountHash,
  ]);
  return ethers.solidityPacked(["address", "bytes"], [admin.target, adminData]);
};

export function genZkAdminSignature(jwt: JwtInput, proof: OidcZkProof): string {
  const jwtSignature =
    "0x" + base64url.toBuffer(jwt.jwtSignature).toString("hex");
  const validationData = ethers.AbiCoder.defaultAbiCoder().encode(
    ["tuple(tuple(bytes32, string, bytes32, bytes), bytes32, bytes)"],
    [
      [
        [jwt.kidSha256, jwt.iat, jwt.jwtHeaderAndPayloadHash, jwtSignature],
        proof.verifier_digest,
        proof.proof,
      ],
    ]
  );
  return ethers.solidityPacked(["uint8", "bytes"], [0, validationData]);
}
