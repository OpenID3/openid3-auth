...
const encryptWithDevKey = (plaintext: string, aad: Buffer) => {
  const key = Buffer.from(process.env.DEV_KEY!, "hex");
  const iv = Buffer.from(process.env.DEV_KEY_IV!, "hex");
...
const jwtSignerConfig = () => ({
  projectId: process.env.FIREBASE_PROJECT_ID!,
  locationId: process.env.GCP_KEY_LOCATION_GLOBAL!,
  keyRingId: process.env.JWT_SIGNER_KEY_RING_ID!,
  keyId: process.env.JWT_SIGNER_KEY_ID!,
  versionId: process.env.JWT_SIGNER_KEY_VERSION_ID!,
});
