...
import { registerUser, getAuth, postAuth, preAuth, Session } from "./firestore";
 ...
 * }
 */
export const registerUserWithPasskey = async (
  req: express.Request,
  res: express.Response
) => {
  if (process.env.ENV !== "dev" && (await registerRateLimit(req.ip || ""))) {
 ...
 * }
 */
export const getDeks = async (req: express.Request, res: express.Response) => {
  const sessionCookie = cookie.parse(req.headers.cookie || "");
  const session = sessionCookie.__session;
  if (!session) {
...
const signJwt = async (
  ...
  const payload = base64url(
    JSON.stringify({
      sub: address,
      iss: process.env.REACT_APP_DOMAIN,
      aud: process.env.REACT_APP_DOMAIN,
      iat: epoch(),
      exp: epoch() + ttl,
      session: sessionId,
    })
...
const verifyJwt = async (
  ...
  if (
    parsedPayload.iss !== process.env.REACT_APP_DOMAIN ||
